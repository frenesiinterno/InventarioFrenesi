...
class SiigoOcModel {
  ...
  // Obtener todas las OCs (una fila por documento)
  static async getAll() {
    const query = `
      SELECT 
        s.*,
  ...
  // Obtener OCs agrupadas por numero_oc (una fila por orden, evita duplicados en lista)
  static async getAllGroupedByNumeroOc() {
    const query = `
      SELECT 
        numero_oc,
        MIN(id) as id,
        MAX(fecha_oc) as fecha_oc,
        MAX(cliente_nombre) as cliente_nombre,
        SUM(total_items) as total_items,
        SUM(items_procesados) as items_procesados,
        MAX(total_pagar) as total_pagar,
      ...
      FROM (
        ...
        FROM siigo_ocs s
        LEFT JOIN siigo_oc_items si ON s.id = si.siigo_oc_id
        GROUP BY s.id, s.numero_oc, s.fecha_oc, s.cliente_nombre, s.total_pagar, s.estado
      ) sub
      GROUP BY numero_oc
      ORDER BY MAX(fecha_oc) DESC
      LIMIT 500
    `;
    const [rows] = await db.execute(query);
    return rows;
    ...
    try {
      ...
      const [items] = await connection.execute(
        `SELECT 
         ...
         LEFT JOIN productos p ON ft.producto_id = p.id
