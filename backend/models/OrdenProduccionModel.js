...
class OrdenProduccionModel {
  ...
  // Obtener orden por siigo_oc_id (para agrupar items de una OC en una sola OP)
  static async getBySiigoOcId(siigoOcId) {
    if (!siigoOcId) return null;
    const query = `SELECT * FROM ordenes_produccion WHERE siigo_oc_id = ? LIMIT 1`;
    const [rows] = await db.execute(query, [siigoOcId]);
    return rows[0] || null;
  }
  ...
  static async create(data) {
    const query = `
      ...
      (numero_orden, producto_id, cantidad_producir, fecha_orden, estado, observaciones, siigo_oc_id, origen_pdf, total_costo_mp, pdf_ruta)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const [result] = await db.execute(query, [
      ...
      data.origen_pdf || null,
      data.total_costo_mp ?? null,
      data.pdf_ruta || null
    ]);
  ...
  static async update(id, data) {
    ...
    if (data.origen_pdf !== undefined) {
      updates.push('origen_pdf = ?');
      values.push(data.origen_pdf);
    }
    if (data.total_costo_mp !== undefined) {
      updates.push('total_costo_mp = ?');
      values.push(data.total_costo_mp);
    }
    if (data.pdf_ruta !== undefined) {
      updates.push('pdf_ruta = ?');
      values.push(data.pdf_ruta);
    }
