...
class OrdenProduccionPDFService {
  ...
  /**
   * Guarda el PDF en disco. Nombre: OP-5546-COSTEADA.pdf (un PDF por orden).
   * @param {Buffer} buffer
   * @param {string} numeroOrden - ej. "OP5546" o "5546"
   * @param {boolean} costeada - si true usa sufijo -COSTEADA (default true)
   */
  static async guardarPDFEnDisco(buffer, numeroOrden, costeada = true) {
    const uploadsDir = path.join(__dirname, '../..', 'uploads', 'ordenes_produccion');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    const num = (numeroOrden || '').replace(/\D/g, '') || numeroOrden;
    const sufijo = costeada ? '-COSTEADA' : '';
    const nombreArchivo = `OP-${num}${sufijo}.pdf`;
