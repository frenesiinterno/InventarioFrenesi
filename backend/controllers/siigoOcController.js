...
exports.getAll = async (req, res) => {
  try {
    const ocs = await SiigoOcModel.getAll();
...
// Procesar item (crear orden de producción y descontar inventario)
exports.procesarItem = async (req, res) => {
  try {
    ...
    // Obtener ficha técnica y producto
    ...
    const [materiales] = await db.execute(
      `SELECT ft.materia_prima_id, ft.consumo as cantidad, mp.nombre as materia_nombre, mp.stock_actual
       FROM fichas_tecnicas ft
    ...
    const ordenId = await OrdenProduccionModel.create({
      numero_orden: `OP-${item.numero_oc}-${item.item_numero}`,
      producto_id: ficha.producto_id,
      cantidad_producir: item.cantidad,
      fecha_orden: item.fecha_oc || new Date(),
      cliente_id: null,
      cliente_nombre: item.cliente_nombre || 'Cliente SIIGO',
      siigo_oc_id: item.siigo_oc_id,
      estado: 'EN_PROCESO',
      observaciones: `Generada desde SIIGO OC: ${item.numero_oc} - Item ${item.item_numero}`
    });

    // Agregar item a la orden
    await db.execute(
      `INSERT INTO op_items 
       (orden_produccion_id, referencia_prenda, cantidad)
       VALUES (?, ?, ?)`,
      [
        ordenId,
        referencia_prenda || `${ficha.producto_nombre} - ${item.talla || ''}`,
        item.cantidad
      ]
    );

    // Procesar orden (descontar materiales del Kardex)
    await OrdenProduccionModel.procesarOrden(ordenId);
    ...
    res.json({
      ...
      message: 'Item procesado correctamente. Los materiales han sido descontados del inventario.'
    });
  } catch (error) {
