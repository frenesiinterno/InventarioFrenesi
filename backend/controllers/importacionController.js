...
const ensureProducto = async (nombreRaw, caches, stats) => {
  ...
  // Usar PrendaModel que opera sobre la tabla `prendas` existente
  let producto = await PrendaModel.getByNombre(nombreNormalizado);
  if (!producto) {
    // Si el sistema desea códigos nulos, los dejamos como NULL para completarlos más tarde.
    const id = await PrendaModel.create({ nombre: nombreNormalizado, codigo: null, activo: true });
    stats.productosCreados += 1;
    producto = await PrendaModel.getById(id);
  }
...
const ensureMateria = async (nombreRaw, tipo, unidad, precioRaw, caches, stats) => {
  ...
  if (!materia) {
    const id = await MateriaPrimaModel.create({
      ...
      tipo_id: (tipo && tipo.id) ? tipo.id : null,
      unidad_medida_id: (unidad && unidad.id) ? unidad.id : null,
  ...
  const materiaStats = {
    ...
    for (const row of rows) {
      ...
      if (!productosLimpiados.has(producto.id)) {
        await FichaTecnicaModel.deleteByPrenda(producto.id);
      ...
      const tipo = await ensureTipo(tipoCol, caches, stats);
      const unidad = await ensureUnidad(unidadCol, caches, stats);
      // No consideramos el precio unitario desde las fichas técnicas (se ignora)
      const materia = await ensureMateria(materiaNombre, tipo, unidad, null, caches, materiaStats);
      ...
      await FichaTecnicaModel.create({
        ...
        consumo: cantidad,
        unidad: (unidad && unidad.codigo) ? unidad.codigo : (unidad && unidad.nombre) ? unidad.nombre : 'UND'
      });
