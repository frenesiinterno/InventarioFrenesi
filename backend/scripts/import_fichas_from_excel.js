    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (row[0] && !row[0].toString().startsWith('Total')) {
        ...
        if (nombrePrenda !== prendaActual) {
          ...
          // Buscar o crear prenda
          const [prendas] = await connection.execute(
            'SELECT id FROM prendas WHERE UPPER(nombre) = ?',
            [nombrePrenda]
          );

          if (prendas.length > 0) {
            productoId = prendas[0].id;
            stats.prendasActualizadas++;
            console.log(`  ✓ Prenda encontrada (ID: ${productoId})`);
          } else {
            // Crear nueva prenda
            const [result] = await connection.execute(
              'INSERT INTO prendas (nombre, codigo, activo) VALUES (?, ?, 1)',
              [nombrePrenda, `PRENDA-${Date.now()}`]
            );
            productoId = result.insertId;
            stats.prendasCreadas++;
            console.log(`  ✓ Prenda creada (ID: ${productoId})`);
          }
        }
      }
      ...
      if (productoId && row[2]) {
        ...
        try {
          ...
          let [materias] = await connection.execute(
            'SELECT id FROM materias_primas WHERE UPPER(nombre) = ?',
            [nombreMateria]
          );
          ...
          if (materias.length > 0) {
            ...
            const [result] = await connection.execute(
              `INSERT INTO materias_primas
               (nombre, unidad_base, stock_minimo, activo)
               VALUES (?, ?, 0, 1)`,
              [nombreMateria, unidadMedida]
            );
            materiaId = result.insertId;
            stats.materiasCreadas++;
          }

          // 4. Crear o actualizar ficha técnica SIN precios (solo cantidades)
          const [fichasExistentes] = await connection.execute(
            'SELECT id FROM fichas_tecnicas WHERE prenda_id = ? AND materia_prima_id = ?',
          ...
          if (fichasExistentes.length > 0) {
            ...
            await connection.execute(
              'UPDATE fichas_tecnicas SET consumo = ?, unidad = ? WHERE id = ?',
              [cantidad, unidadMedida, fichasExistentes[0].id]
            );
            stats.fichasActualizadas++;
          } else {
            // Crear
            await connection.execute(
              'INSERT INTO fichas_tecnicas (prenda_id, materia_prima_id, consumo, unidad) VALUES (?, ?, ?, ?)',
              [productoId, materiaId, cantidad, unidadMedida]
            );
            stats.fichasCreadas++;
          }
